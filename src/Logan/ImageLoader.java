...
public class ImageLoader {
	...
	public static PImage RED_PROJECTILE, GREEN_PROJECTILE, YELLOW_STAR, PURPLE_STAR, MAIN_CHARACTER_LEFT,
			MAIN_CHARACTER_RIGHT, BOSS_PILLAR, BOSS,MINION_RIGHT,MINION_LEFT,SLIME_RIGHT,SLIME_LEFT,GAME_OVER,BLUE_PROJECTILE;
	...
	public static void setUp(PApplet p) {
		...
		BLUE_PROJECTILE = p.loadImage("Images" + fs + "Projectiles" + fs + "Blue Projectile.gif");
		...
		for(int i = 0; i < 13; i ++) {
			EXPLOSION[i] = p.loadImage("Images" + fs + "Explosion" + fs + (i + 1) + ".png");
		}
		GAME_OVER = p.loadImage("Images" + fs + "GG" + fs + "Game Over.gif");
